...
class Snake:
    ...
    def check_collision(self, food):
        ...
        if head.x == food.pos.x and head.y == food.pos.y:
            ...
            if SCORE >= REACH + 5: #every 5 points increases fps which increases speed
                LEVEL += 1
                FPS += 1
                REACH += 5
...
class Food:
    ...
    def generate_random_pos(self, snake):
        while True:
            ...
            if not any(segment.x == self.pos.x and segment.y == self.pos.y for segment in snake.body): #if on body of snake, then regenerates position
                break   
